# Project Rules — turbo-memory-mcp
...
Python 서버(`server.py`) 기본 임베딩 모델:
- **`intfloat/multilingual-e5-small`** (384-dim, 한국어/다국어 지원)
- E5 규칙: 저장은 `passage: ` prefix, 검색은 `query: ` prefix 자동 적용됨

Rust 서버(`server-rs/`)는 모델-agnostic — 클라이언트가 임베딩해서 전송:
- `tools/add`에 `embedding: Vec<f32>` 필수
- `tools/search`에 `query_vec: Vec<f32>` 필수
- 차원만 일치하면 어떤 임베딩 모델이든 수용
